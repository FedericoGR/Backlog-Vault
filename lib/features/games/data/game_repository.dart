    ...
    return _sync.run(
      ...
      action: (_) async {
        ...
        for (final entryId in ids) {
          ...
          await (_db.update(_db.games)
            ..where((table) => table.id.equals(entry.gameId))).write(
      ...
      action: (_) async {
        ...
        final existing =
            await ((_db.select(_db.playthroughs)
                  ...
                  ..where(
                    (table) =>
                        table.libraryEntryId.equals(model.libraryEntryId),
                  )
                  ..where((table) => table.deletedAt.isNull()))
                .getSingleOrNull());
        if (existing == null) {
      ...
      action: (_) async {
        ...
        if (activePlaythrough == null) {
          ...
          await (_db.update(_db.playthroughs)
            ...
            PlaythroughsCompanion(
              platformId: Value(
                model.platformId ?? activePlaythrough.platformId,
              ),
              ...
              rating: Value(model.rating),
              notes: Value(
                _blankToNull(model.notes) ?? activePlaythrough.notes,
              ),
              updatedAt: Value(now),
            ),
          );
